  ...
  <div class="login-page">
    <div class="brand-panel">
      <div class="brand-content">
        ...
        <h1 class="brand-title">医生工作台</h1>
        <p class="brand-subtitle">专业医生在线接诊系统</p>
...
const handleLogin = async () => {
  ...
  try {
    const success = await doctorStore.login(username.value, password.value)
    if (success) {
      router.push('/doctor/dashboard')
    } else {
      errorMessage.value = '登录失败，请检查用户名'
    }
  } catch (error) {
...
.feature-item:nth-child(2) { animation-delay: 0.27s; }
